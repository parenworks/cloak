  ...
  (let* ((username (cdr (assoc :username params)))
    ...
    (when net-cfg
      (let ((name (cdr (assoc :name params)))
            ...
            (sasl-account (cdr (assoc :sasl-account params)))
        ...
        (setf (config:network-sasl net-cfg)
              (when (and sasl-str (plusp (length sasl-str)))
                (intern (string-upcase sasl-str) :keyword)))
        (setf (config:network-sasl-account net-cfg)
              (when (and sasl-account (plusp (length sasl-account))) sasl-account))
        (when buf-str
